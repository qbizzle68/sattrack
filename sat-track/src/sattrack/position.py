...
def nearestTrueAnomaly(sat: Satellite, time: JulianDate, trueAnom: float) -> JulianDate:
    ...
    meanAnom0 = trueToMean(computeTrueAnomaly(state[0], state[1]), sat.getTle().getEcc())
    meanAnom1 = trueToMean(trueAnom, sat.getTle().getEcc())
    if meanAnom1 < meanAnom0:
        ...
        if meanAnom1 - meanAnom0 > pi:
            ...
            dma = meanAnom1 - meanAnom0
    n = sat.getTle().getMeanMotion() * 2 * pi
