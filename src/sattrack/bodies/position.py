...
class JulianTimes:
    __slots__ = '_jd', '_jde', '_jc', '_jce', '_jme', '_hash'
...
@lru_cache(maxsize=8)
def _computeNutationTrigTerm(time: 'JulianDate'):
    ...
    return [sum((xi * yi for xi, yi in zip(xTerms, yRow))) for yRow in SIN_TABLE]
...
@lru_cache(maxsize=8)
def computeNutationLongitudeDelta(time: 'JulianDate') -> float:
    if isinstance(time, JulianTimes):
        JCE = time.JCE
    else:
        JDE = time.value + (DELTAT / 86400)
    ...
    # x4 = computeRaanMoon(time)
    # xTerms = (x0, x1, x2, x3, x4)
    # trigTerms = _computeNutationTrigTerm(xTerms)
    trigTerms = _computeNutationTrigTerm(time)
    dPsiList = ((a + b * JCE) * sin(trigTerm) for (a, b, *_), trigTerm in zip(NUTATION_TABLE, trigTerms))
    deltaPsi = sum(dPsiList)

    # deltaPsi = 0
    # for (a, b, *_), trigTerm in zip(NUTATION_TABLE, trigTerms):
    #     dPsi_i = (a + b * JCE) * sin(trigTerm)
    #     deltaPsi += dPsi_i

    return radians(deltaPsi / 36000000)


@lru_cache(maxsize=8)
def computeNutationObliquityDelta(time: 'JulianDate') -> float:
    if isinstance(time, JulianTimes):
        JCE = time.JCE
    else:
        JDE = time.value + (DELTAT / 86400)
    ...
    trigTerms = _computeNutationTrigTerm(time)
    dEpsilonList = ((c + d * JCE) * cos(trigTerm) for (*_, c, d), trigTerm in zip(NUTATION_TABLE, trigTerms))
    deltaEpsilon = sum(dEpsilonList)
    ...
    return radians(deltaEpsilon / 36000000)
...
def computeTrueObliquity(time: 'JulianDate') -> float:
    ...
    nutationObliquity = computeNutationObliquityDelta(time)
...
def computeApparentSiderealTime(time: 'JulianDate | JulianTimes') -> float:
    ...
    deltaNutationObliquity = computeNutationObliquityDelta(time)
    deltaNutationLongitude = computeNutationLongitudeDelta(time)
    meanObliquity = computeMeanObliquity(time)
    # trueObliquity = meanObliquity + deltaEpsilon
    trueObliquity = meanObliquity + deltaNutationObliquity
    meanSiderealTime = computeMeanSiderealTime(time)

    # return (meanSiderealTime + deltaPsi * cos(trueObliquity)) % TWOPI
    return (meanSiderealTime + deltaNutationLongitude * cos(trueObliquity)) % TWOPI
